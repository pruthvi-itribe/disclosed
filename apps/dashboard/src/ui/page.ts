 ...
 *
 * AND THE PANEL IS NOW A DEPLOYMENT DECISION AS WELL AS A LAYOUT ONE. `admin`
 * decides whether it is in this document at all — the tab, the section, the six
 * filter selects it carries and the client fragment that drives them. Off, none
 * of it is here: not hidden, not disabled, ABSENT. The flag is the server's
 * (`configuration.ts`), never inferred in the browser, because a page that
 * decided this for itself would have to ship the panel to decide against it.
 * The markup lives in `page-admin.ts`.
 *
 ...
 */
export const renderDashboardPage = (admin: boolean): string => `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<header class="topbar" data-ui="top-bar">
  ...
  <nav class="tabs" role="tablist">
    ...
    ${admin ? ADMIN_TAB : ADMIN_TAB_ABSENT}
  </nav>
  <!--
...
<script>${pageScript(admin)}</script>
</body>
</html>
`;
